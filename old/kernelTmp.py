...
def validLine(line):
    return not line.startswith('#') \
            and not line.startswith('NA')\
            and not line.startswith('undef')
'''
def buildMatrixFromKsFile(filePath):
    KS = pd.DataFrame()
    with open(filePath) as f:
        l = f.readlines()
        l = filter(validLine,l)
        l = [i.replace('\n','') for i in l]
        for line in l:
            line = line.split('\t')
            genome_chr1 = line[2][1:]
            
            KS[][]



buildMatrixFromKsFile('./ks/real/4241_4242.CDS-CDS.last.tdd10.cs0.filtered.dag.all.go_D20_g10_A5.aligncoords.gcoords.ks')
'''
